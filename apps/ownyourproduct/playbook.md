...
Auth, Admin) are the wizard's; the overlay adds `src/modules/comment-studio/`.
...
Edit copy there, never inline in components. The default keyword lists per
language are in `comment-studio-model.ts`.
...
## 1. Register the Comment Studio module
...
1. In `src/modules/index.ts`: import `moduleDescriptor as commentStudio` from
   `./comment-studio` and put it FIRST in `MODULES`. The shell sends `/` to
   the first rail module, so this makes Comment Studio the landing screen.
2. In `src/modules/navGroups.tsx`: add `'comment-studio'` to the items of the
   `ai` group, before `'automations'`.

Verify: `npm run check` passes, `/` redirects to `/comment-studio`, and with
ES selected the screen renders the header "Comment Studio" with the status tag
"Apagado" and the language switch ES / PT / EN.

## 2. Run the module's tests and the typecheck

The overlay ships its own tests: the write plan, the runner's create-then-shape
contract, the read-back of the bot, the generated texts, and a white-screen
guard.

Verify: `npx vitest run src/modules/comment-studio` reports every test
passed, and `npm run check` is clean.

## 3. Connect Instagram and turn Comment Studio on
...
## 4. Sign-up lands in Comment Studio

A new salon that signs up through the auth module gets its bot from the
wizard's sign-up flow and is sent to `/`, which step 1 made Comment Studio.
Keep it that way: the first minute must be "connect Instagram, press
Encender", not a tour of seven modules.

Verify: sign up with a fresh e-mail; the first screen is Comment Studio with
the "Conecta tu Instagram" notice and a "Ir a Canales" button that opens
Channels.
