...
Modules: `bookings` (calendar, services, staff), `contacts`, `knowledge-base`, `automations`, `channels` (Instagram, WhatsApp, the web widget), `auth` (accounts) and `admin` — the four product screens ship in this app's overlay under `src/modules/`.
